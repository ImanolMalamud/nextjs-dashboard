...
export async function fetchFilteredRegisters(
  query: string,
  currentPage: number,
) {
  noStore();
...
export async function fetchRegistersPages(query: string) {
  noStore();
  ...
  `;
    ...
    console.error('Database Error:', error);
    throw new Error('Failed to fetch total number of registers.');
  }
}
